...
@php
    $filters = [
        '' => ['ทั้งหมด', $rows->total()],
        'PENDING' => ['รออนุมัติ', $statusCounts['pending'] ?? 0],
        'APPROVED' => ['อนุมัติแล้ว', $statusCounts['approved'] ?? 0],
        'REJECTED' => ['ไม่อนุมัติ', $statusCounts['rejected'] ?? 0],
    ];
@endphp
...
<div class="space-y-5">
    ...
    <div class="grid gap-3 sm:grid-cols-3">
        <div class="metric-card"><span class="text-xs text-slate-500">รอ Admin อนุมัติ</span><strong class="mt-2 block text-2xl text-amber-600">{{ $statusCounts['pending'] ?? 0 }}</strong></div>
        <div class="metric-card"><span class="text-xs text-slate-500">อนุมัติแล้ว</span><strong class="mt-2 block text-2xl text-emerald-600">{{ $statusCounts['approved'] ?? 0 }}</strong></div>
        <div class="metric-card"><span class="text-xs text-slate-500">ไม่อนุมัติ</span><strong class="mt-2 block text-2xl text-rose-600">{{ $statusCounts['rejected'] ?? 0 }}</strong></div>
    </div>
    ...
    <div class="panel overflow-hidden">
        ...
        <div class="table-wrap">
            <table class="data-table">
                ...
                @forelse($rows as $row)
                @php
                    $pdfReady = method_exists($row, 'isReadyForPdf') ? $row->isReadyForPdf() : false;
                    $displayProduct = $row->targetProduct ?? $row->items?->first()?->product;
                    $isAdminCreated = $row->requester?->isAdmin() ?? false;
                    $statusVal = $row->status?->value ?? (string)$row->status;
                    $step = match(true) {
                        $statusVal === 'REJECTED' => ['ไม่อนุมัติ', 'badge-red'],
                        $statusVal === 'APPROVED' => ['อนุมัติแล้ว', 'badge-green'],
                ...
                <tr>
                    ...
                    <td><strong class="block text-xs text-slate-800">{{ $typeLabel }}</strong>@if($isAdminCreated)<span class="mt-1 inline-block text-[10px] text-blue-600">สร้างโดย Admin</span>@endif</td>
                    ...
                    <td><div class="flex items-center gap-2"><x-product-image :product="$displayProduct" size="sm" /><span class="text-xs text-slate-600">{{ $row->targetProduct ? $row->targetProduct->name.' × '.\App\Support\Quantity::format($row->target_quantity) : ($row->items_count ?? 0).' รายการ' }}</span></div></td>
