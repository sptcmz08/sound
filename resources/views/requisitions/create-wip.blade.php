...
<div class="mx-auto max-w-6xl space-y-6">
    <div class="flex flex-wrap items-end justify-between gap-4">
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <p class="page-subtitle">ตั้งชื่อ WIP แล้วเลือกรายการ PART ที่กำหนดจำนวนต่อ WIP 1 ชิ้นได้ชัดเจน</p>
            </div>
        </div>
    ...
    <form method="post" action="{{ route('requisitions.wip.store') }}" enctype="multipart/form-data" class="space-y-6" id="wip-form">
        ...
        <section class="panel">
            <div class="panel-header">
                <div><h3 class="text-xl font-bold text-slate-950">2. รายการชิ้นส่วนที่ใช้ (PART)</h3><p class="mt-0.5 text-sm text-slate-500">เลือก PART และจำนวนที่ใช้ต่อ WIP 1 {{ $unit->name }} — SUPPLY ไม่รวมใน BOM</p></div>
...
function options(selected) {
    return '<option value="">— เลือกชิ้นส่วน PART —</option>' + parts.map(part =>
